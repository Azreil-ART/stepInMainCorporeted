...
angular
  ...
  .config([
    ...
    function ($provide) {
      ...
      $provide.decorator("$httpBackend", [
        ...
        "$window",
        "uploadManager",
        function ($delegate, $window, uploadManager) {
      ...
      var hasFile = function (data) {
        for (var n in data) {
          ...
          if (
            (angular.isObject(data[n]) || angular.isArray(data[n])) &&
            hasFile(data[n])
          ) {
            return true;
          }
      ...
      $provide.decorator("$http", [
        ...
        function ($delegate) {
          ...
          $delegate.defaults.transformRequest = [
            function (data) {
        ...
        register: function (xhr) {
          ...
          xhr.upload.onprogress = function (e) {
            if (e.lengthComputable) {
              ...
              self.uploads[xhr._idXhr]["percent"] = Math.round(
  ...
  .directive("type", [
    "$parse",
    function urModelFileFactory($parse) {
      /**
      ...
      return {
        ...
        link: function urFilePostLink(scope, element, attrs, ngModel) {
          ...
          element.bind("change", function (e) {
            ...
            var index,
              fileData = attrs.multiple ? e.target.files : e.target.files[0];
  ...
  .directive("dropTarget", [
    "$parse",
    "fileHandler",
    function urDropTargetFactory($parse, fileHandler) {
      return {
        scope: false,
        restrict: "EAC",
        require: "?ngModel",
